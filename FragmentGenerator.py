...
class FragmentGenerator:
    ...
    def __init__(
        ...
        idxs = zip(self.otus_idx, self.regions_idx)
    ...
    def generate(
        ...
        if self.debug:
            print(f"Generating fragments for sample {filename}...")
        if ratios is None:
            if self.debug:
        ...
        records = [
            self.noiser(record, phred_params=phred_params)
            for i, record in enumerate(SeqIO.parse(self.fasta_path, "fasta"))
            if i in chosen_idxs
        ]
        if self.debug:
            print(f"Writing records to file {filename}...")
        with open(filename, "w") as out:
            SeqIO.write(records, out, "fastq")
        if self.debug:
            print(f"Sample {filename} generated.")
        return ratios
